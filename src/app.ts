...
async function startServer(): Promise<void> {
  try {
    ...
    // Initialize services (don't hard-fail if Binance init fails)
    try {
      await serviceManager.initialize();
    } catch (initErr) {
      console.warn(
        "⚠️ Service initialization failed, continuing without Binance services:",
    ...
    try {
      ...
      console.warn(
        "⚠️ RAG service initialization failed:",
        error instanceof Error ? error.message : error
      );
      console.warn(
        "RAG endpoints will not be available until service is initialized"
      );
    }
    ...
    server.listen(config.port, "0.0.0.0", () => {
      ...
      if (config.nodeEnv === "development") {
        ...
        console.log(
          `📈 Spot API: http://localhost:${config.port}/api/binance/spot/`
        );
        console.log(
          `⚡ Futures API: http://localhost:${config.port}/api/binance/futures/`
        );
        console.log(
          `🪙 COIN-M API: http://localhost:${config.port}/api/binance/coinm/`
        );
        console.log(
          `🔌 WebSocket API: http://localhost:${config.port}/api/binance/websocket/`
        );
        console.log(`🤖 RAG API: http://localhost:${config.port}/api/rag/`);
        console.log(
          `📸 Instagram API: http://localhost:${config.port}/api/instagram/`
        );
      }
    });
  } catch (error) {
