...
export class FirebaseInstagramService {
  ...
  async getAllPosts(): Promise<InstagramPost[]> {
    try {
      ...
      snapshot.docs.forEach((doc) => {
        ...
        const convertedPost =
          this.convertFirestoreToInstagramPost(firestorePost);
        posts.push(convertedPost);
      });

      logger.info(
        `Successfully fetched ${posts.length} Instagram posts from Firestore`
      );
      return posts.sort(
        (a, b) =>
  ...
  async getPostsPaginated(
    ...
    try {
      logger.info("Fetching paginated Instagram posts from Firestore", {
        limit,
        startAfter,
      });
      ...
      logger.info(
        `Successfully fetched ${posts.length} paginated Instagram posts from Firestore`
      );
      return { posts, hasMore, lastPostId };
    } catch (error) {
      logger.error(
        "Error fetching paginated Instagram posts from Firestore:",
        error
      );
  ...
  async getPostById(postId: string): Promise<InstagramPost | null> {
    try {
      ...
      logger.debug("Successfully fetched Instagram post by ID from Firestore", {
        postId,
      });
      return convertedPost;
    } catch (error) {
      logger.error(
        "Error fetching Instagram post by ID from Firestore:",
        error
      );
  ...
  async getAnalytics(postsLimit?: number): Promise<InstagramAnalytics> {
    try {
      logger.info("Generating Instagram analytics from Firestore data", {
        postsLimit,
      });
      ...
      const contentPerformance = posts.map((post) => ({
        ...
        benchmarks: {
          avgLikes:
            posts.reduce((sum, p) => sum + p.metrics.likesCount, 0) /
            posts.length,
          avgComments:
            posts.reduce((sum, p) => sum + p.metrics.commentsCount, 0) /
            posts.length,
      ...
      logger.info(
        "Successfully generated Instagram analytics from Firestore data",
        {
      ...
      logger.error(
        "Error generating Instagram analytics from Firestore:",
        error
      );
  ...
  async getRawFirestoreData(): Promise<{
    ...
    try {
      ...
      logger.info(
        `Successfully fetched ${posts.length} raw Instagram posts from Firestore`
      );
      return result;
    } catch (error) {
   ...
   */
  private convertFirestoreToInstagramPost(
    firestorePost: FirestoreInstagramPost
  ): InstagramPost {
    ...
    const metrics: InstagramMetrics = {
      ...
      ...(isVideo &&
        firestorePost.metrics.ig_reels_avg_watch_time && {
    ...
    return {
      ...
      mediaType,
      mediaUrl: firestorePost.mediaUrl || "",
      thumbnailUrl: firestorePost.thumbnailUrl || "",
  ...
  private calculateEngagementRate(metrics: FirestoreInstagramMetrics): number {
    ...
    return (
      ((metrics.likes + metrics.comments + metrics.saved + metrics.shares) /
        metrics.reach) *
      100
    );
  }
  ...
  async savePost(post: InstagramPost): Promise<void> {
    try {
      ...
      const firestorePost: FirestoreInstagramPost =
        this.convertInstagramPostToFirestore(post);
      const cleanedPost = this.cleanUndefinedValues(firestorePost);

      await this.db
        .collection(this.COLLECTION_NAME)
        .doc(post.id)
        .set(cleanedPost, { merge: true });
  ...
  async savePosts(posts: InstagramPost[]): Promise<void> {
    try {
      logger.info("Saving Instagram posts to Firestore in batch", {
        count: posts.length,
      });
      ...
      logger.error(
        "Error saving Instagram posts to Firestore in batch:",
        error
      );
  ...
  async updatePostMetrics(
    ...
    try {
      ...
      const firestoreMetrics: FirestoreInstagramMetrics =
        this.convertMetricsToFirestore(metrics);
      ...
      logger.error(
        "Error updating Instagram post metrics in Firestore:",
        error
      );
  ...
  async saveAnalyticsSummary(analytics: InstagramAnalytics): Promise<void> {
    try {
      ...
      await this.db
        ...
        .doc("latest")
        .set(cleanedAnalytics, { merge: true });

      logger.info(
        "Successfully saved Instagram analytics summary to Firestore",
        {
          postsAnalyzed: analytics.posts.length,
      ...
      logger.error(
        "Error saving Instagram analytics summary to Firestore:",
        error
      );
  ...
  async deletePost(postId: string): Promise<void> {
    try {
      ...
      logger.info("Successfully deleted Instagram post from Firestore", {
        postId,
      });
    } catch (error) {
  ...
  async postExists(postId: string): Promise<boolean> {
    try {
      ...
      logger.error(
        "Error checking if Instagram post exists in Firestore:",
        error
      );
      return false;
    }
   ...
   */
  async getPostsNeedingMetricsUpdate(
    hoursOld: number = 24
  ): Promise<InstagramPost[]> {
    try {
      logger.info("Finding Instagram posts needing metrics update", {
        hoursOld,
      });
      ...
      logger.info(
        `Found ${posts.length} Instagram posts needing metrics update`
      );
      return posts;
    } catch (error) {
      logger.error(
        "Error finding Instagram posts needing metrics update:",
        error
      );
   ...
   */
  private convertInstagramPostToFirestore(
    post: InstagramPost
  ): FirestoreInstagramPost {
    return {
      id: post.id || "",
   ...
   */
  private convertMetricsToFirestore(
    metrics: InstagramMetrics
  ): FirestoreInstagramMetrics {
    return {
      ...
      total_interactions:
        ...
        (metrics.likesCount || 0) +
          ...
          (metrics.sharesCount || 0) +
          (metrics.savesCount || 0),
   ...
   */
  private mapMediaTypeToFirestore(
    mediaType: string
  ): "FEED" | "REEL" | "STORY" {
  ...
  async postsExist(postIds: string[]): Promise<{ [postId: string]: boolean }> {
    try {
      logger.debug("Checking existence of multiple posts in Firestore", {
        count: postIds.length,
      });

      // Firestore batch get operation
      const docRefs = postIds.map((id) =>
        this.db.collection(this.COLLECTION_NAME).doc(id)
      );
      ...
      logger.error(
        "Error checking multiple posts existence in Firestore:",
        error
      );
  ...
  async getPostsMissingThumbnailUrls(postIds: string[]): Promise<string[]> {
    try {
      logger.debug("Checking for posts missing thumbnail URLs", {
        count: postIds.length,
      });
   ...
   */
  async updatePostThumbnailUrl(
    postId: string,
    thumbnailUrl: string
  ): Promise<void> {
    try {
      logger.debug("Updating post thumbnail URL in Firestore", {
        postId,
      ...
      throw new Error(
        `Failed to update thumbnail URL for post ${postId}: ${error}`
      );
    }
  }

  /**
   * Update a post's mediaUrl in Firestore
   */
  async updatePostMediaUrl(postId: string, mediaUrl: string): Promise<void> {
    try {
      logger.debug("Updating post media URL in Firestore", {
        postId,
      ...
      logger.debug("Successfully updated post media URL", { postId });
    } catch (error) {
      logger.error("Error updating post media URL in Firestore:", error);
      throw new Error(
        `Failed to update media URL for post ${postId}: ${error}`
      );
    }
  }
}
